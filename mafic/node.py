...
class Node(Generic[ClientT]):
    ...
    async def _ws_listener(self) -> None:
        ...
        while True:
            ...
            _log.debug("Received message from websocket: %s", msg.type, extra={"label": self._label})
    ...
    async def _handle_msg(self, data: IncomingMessage) -> None:
        ...
        data:
            The data to handle.
        """
        _log.debug("Handling message op: %s", data.get("op"), extra={"label": self._label})
        if not isinstance(data, dict) or "op" not in data:
            _log.warning("Received message without 'op' key: %s", data, extra={"label": self._label})
            return
